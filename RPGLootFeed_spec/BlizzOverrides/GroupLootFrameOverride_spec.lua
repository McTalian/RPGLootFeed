...
describe("GroupLootFrameOverride module", function()
	...
	describe("escape hatch / on-demand access", function()
		-- Documents the ACTUAL behavior of the suppression mechanism.
		-- There is currently NO escape hatch: /loot and Alt+Click are both
		-- intercepted identically to game-triggered Show calls.

		it("slash command /loot is suppressed when disableLootRollFrame is true", function()
			-- When suppression is enabled, calling frame:Show() directly — as the
			-- built-in /loot command does — is still intercepted and suppressed.
			-- The frame stays hidden; no escape hatch exists.
			ns.DbAccessor.AnyFeatureConfig = function(_, key)
				if key == "lootRolls" then
					return { disableLootRollFrame = true }
				end
			...
			-- Simulate /loot → GroupLootFrame:Show() → hook fires
			GroupLootFrameOverride:InterceptGroupLootFrame(frame1)
			-- Original NOT called: suppression wins even for manual /loot invocation.
			assert.is_false(originalCalled, "expected /loot-triggered Show to be suppressed")
		end)

		it("slash command /loot is NOT suppressed when disableLootRollFrame is false", function()
			-- When suppression is disabled the frame shows normally whether the
			-- trigger is the game engine or the user's /loot command.
			ns.DbAccessor.AnyFeatureConfig = function(_, key)
				if key == "lootRolls" then
					return { disableLootRollFrame = false }
				end
			...
			GroupLootFrameOverride:InterceptGroupLootFrame(frame1)
			assert.is_true(originalCalled, "expected /loot-triggered Show to pass through when suppression disabled")
		end)

		it("Alt+Click bypass is not implemented — user-context Show is suppressed identically to game-triggered Show", function()
			-- There is no Alt+Click handler in the current implementation (MEM075,
			-- deferred feature).  Any Show call, regardless of context, goes through
			-- the same InterceptGroupLootFrame hook and is suppressed if
			-- disableLootRollFrame is true.  This test documents that parity.
			ns.DbAccessor.AnyFeatureConfig = function(_, key)
				if key == "lootRolls" then
					return { disableLootRollFrame = true }
				end
			...
			-- Even if triggered by "user action" context there is no bypass.
			GroupLootFrameOverride:InterceptGroupLootFrame(frame2)
			assert.is_false(originalCalled, "expected user-context Show to be suppressed (no Alt+Click bypass exists)")
		end)
	end)
end)

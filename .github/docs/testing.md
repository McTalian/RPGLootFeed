...
| `Transmog_spec.lua`              | `BuildPayload`/`fromPayload` pattern; `WoWAPI = { Transmog = {} }` ns mock; single `_transmogAdapter`; `spy.on(Transmog, "BuildPayload")` for event handler assertions                                                          |

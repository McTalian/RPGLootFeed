...
function LootDisplay:CreateSampleRows(frame)
	...
	if G_RLF.db.global.item.enableIcon and not G_RLF.db.global.misc.hideAllIcons then
	...
	-- ── PARTY frame: only PartyLoot rows route there ───────────────────────────
	if frame == G_RLF.Frames.PARTY then
		...
		if partyModule and partyModule:IsEnabled() then
			LootElementBase:fromPayload({
				...
				type = "PartyLoot",
				eventChannel = "RLF_NEW_PARTY_LOOT",
				isLink = true,
				unit = "player",
				icon = sampleItemIcon,
	...
	-- 2. Money (uses GenerateTextElements + TextTemplateEngine for real formatting)
	...
	if moneyModule and moneyModule:IsEnabled() then
		...
		local moneyElementData = {
			...
			quantity = quantity,
			icon = (G_RLF.db.global.money.enableIcon and not G_RLF.db.global.misc.hideAllIcons) and DefaultIcons.MONEY
				or nil,
	...
	-- 5. Experience (uses GenerateTextElements + TextTemplateEngine for real formatting)
	...
	if travelModule and travelModule:IsEnabled() then
		local tpDb = G_RLF.db.global.travelPoints
	...
	-- 8. Transmog (Retail only — module's IsEnabled() returns false on Classic)
	...
	if transmogModule and transmogModule:IsEnabled() then
		LootElementBase:fromPayload({
			...
			type = "Transmog",
			isLink = true,
			icon = (G_RLF.db.global.transmog.enableIcon and not G_RLF.db.global.misc.hideAllIcons)
					and DefaultIcons.TRANSMOG
				or nil,
			quality = ItemQualEnum.Epic,
			quantity = 1,
			highlight = true,
